...
typedef enum {EXE_SINGLE, EXE_PARALLEL, EXE_SEQUENTIAL, EXE_REDIRECT, EXE_PIPE}STATUS_CODE;
...
STATUS_CODE parseInput(char *arguments[MAXARGLEN], char** parsedInput[MAXARGLEN], int* parsedInputLen) {
  ...
  while(arguments[i]!=NULL) {
    if(strcmp(arguments[i],"&&")!=0 && strcmp(arguments[i],"##")!=0 && strcmp(arguments[i],">")!=0 && strcmp(arguments[i],"|")!=0) {
      ...
      else if(strcmp(arguments[i],"|")==0) returnStatus=EXE_PIPE;
...
void executePipedCommands(char** parsedInput[MAXARGLEN], int parsedInputLen) {
  int i;
  int fd[(parsedInputLen-1)*2];
  int pid[parsedInputLen];
  for(int i=0;i<parsedInputLen-1;i++) {
    pipe(fd+2*i);
  }
  ...
  for(int i=0;i<parsedInputLen;i++) {
    ...
    if(pid[i]==0) {
      ...
      if(i<parsedInputLen-1)
        dup2(fd[2*i+1],STDOUT_FILENO);
      for(int j=0;j<2*(parsedInputLen-1);j++)
        close(fd[j]);

      execvp(parsedInput[i][0],parsedInput[i]);
      printf("Shell: Incorrect command\n");
      exit(1);
    }
  }
  for(int j=0;j<2*(parsedInputLen-1);j++)
    close(fd[j]);
  for(int j=0;j<parsedInputLen;j++)
...
int main() {
  ...
  char** parsedInput[MAXARGLEN]; int parsedInputLen=0;
    ...
    /**
     * parseInput seperates commands into different string arrays
     * Only one string array for single command
     * Second argument is file in case of redirection
     */
    executionType=parseInput(arguments,parsedInput,&parsedInputLen); 
		
		if(executionType==EXE_PARALLEL) executeParallelCommands(parsedInput,parsedInputLen); //parallel execution
		else if(executionType==EXE_SEQUENTIAL) executeSequentialCommands(parsedInput,parsedInputLen);	// sequential execution
		else if(executionType==EXE_REDIRECT) executeCommandRedirection(parsedInput);	//redirection
    else if(executionType==EXE_PIPE) executePipedCommands(parsedInput,parsedInputLen); //piping
		else executeCommand(parsedInput,0);		//single commands
